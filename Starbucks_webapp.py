...
with tab1:
    ...
    st.write('Please click the tab [Main Page] to actually view the Map and Analysis!')

    st.subheader('Overview')
    st.write("1. Starbucks Location Map")
    st.write('    * Visualizes the number of Starbucks locations across different ZIP codes in Los Angeles. Users can interact with the map to view specific areas. This helps to quickly grasp which areas have higher concentrations of Starbucks stores relative to others.'
    )
    ...
    st.write("    * The full correlation matrix table is shown, allowing users to examine the relationships between all variables at once.")

    st.write("4. Correlations with Number of Starbucks")
    st.write("    * This section presents the sorted correlation values between the number of Starbucks locations and each demographic and health variable in the dataset.")

    st.write("5. Correlation Matrix Heatmap")
    st.write("    * A heatmap of the correlation matrix is displayed, using a color scale to show the strength and direction of correlations. Warm colors represent positive correlations while cool colors represent negative correlations. This visualization allows users to quickly capture the relationships between variables at a glance.")
    ...
    sorted_columns = ['Zip Code'] + sorted_columns  

    df = df[sorted_columns]

    st.dataframe(df)
    
    st.write('    * The analysis of the correlation between Starbucks locations per ZIP code and various health and demographic variables in Los Angeles revealed unexpectedly low correlations, contradicting the initial hypothesis. Most variables, such as physical health, diabetes prevalence, and median household income, showed weak correlation coefficients (ranging from -0.26 to +0.23), suggesting that Starbucks location strategy in Los Angeles may not be significantly influenced by these factors. Future research should consider a broader geographic scope (such as entire United States or Global scale) and additional variables to better understand the factors driving Starbucks location decisions.')

    st.subheader("Major Gotchas or Possible Improvements")

    st.write('1. Improve Starbucks Location Map')
...
with tab2:
    ...
    exclude_columns = ['Zip Code', 'NumberOfStarbucks']  
    ...
    # Sorted correlation values with NumberOfStarbucks
    ...
    st.dataframe(num_starbucks_corr.to_frame().T)  
